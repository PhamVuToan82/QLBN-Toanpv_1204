...
namespace NamDinh_QLBN.Reports.PhauThuat
{
    ...
    /// </summary>
    public partial class rptTH_ThuThuat_PM : DataDynamics.ActiveReports.ActiveReport3
    {
        private object _ThangBC;
        public rptTH_ThuThuat_PM(object ThangBC)
        {
            //
        ...
        private void rptTH_PhuThuat_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("SELECT DMKHOAPHONG.MAKHOA,DMKHOAPHONG.TENKHOA,B.PHIEN_CAPCUU,B.MAPT,C.LOAIPHAUTHUAT FROM "
                ...
                + " INNER JOIN DMKHOAPHONG ON DMKHOAPHONG.MAKHOA = A.MAKHOA"
                + "  WHERE DATEDIFF(MM,B.THOIGIANBD,'{0:MM/dd/yyyy}') = 0 and C.LOAIPHAUTHUAT IN(5,51,52,53) ORDER BY DMKHOAPHONG.MAKHOA,DMKHOAPHONG.TENKHOA", _ThangBC);
            this.DataSource = _ds;
        ...
        private void rptTH_PhuThuat_FetchData(object sender, FetchEventArgs eArgs)
        {
            ...
            else
            {
                ...
                Fields["PTDB_P"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "1" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "5" ? 1 : 0;
                Fields["PTDB_CC"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "2" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "5" ? 1 : 0;

                Fields["PT1A_P"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "1" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "51" ? 1 : 0;
                Fields["PT1A_CC"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "2" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "51" ? 1 : 0;
                ...
                Fields["PT2A_P"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "1" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "52" ? 1 : 0;
                Fields["PT2A_CC"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "2" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "52" ? 1 : 0;
                ...
                Fields["PT3_P"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "1" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "53" ? 1 : 0;
                Fields["PT3_CC"].Value = Fields["PHIEN_CAPCUU"].Value.ToString() == "2" && Fields["LOAIPHAUTHUAT"].Value.ToString() == "53" ? 1 : 0;

                Fields["TongPhien"].Value = Int32.Parse(Fields["PTDB_P"].Value.ToString()) + Int32.Parse(Fields["PT1A_P"].Value.ToString()) + Int32.Parse(Fields["PT2A_P"].Value.ToString())  + Int32.Parse(Fields["PT3_P"].Value.ToString());
                Fields["TongCC"].Value = Int32.Parse(Fields["PTDB_CC"].Value.ToString()) + Int32.Parse(Fields["PT1A_CC"].Value.ToString())  + Int32.Parse(Fields["PT2A_CC"].Value.ToString()) + Int32.Parse(Fields["PT3_CC"].Value.ToString());
