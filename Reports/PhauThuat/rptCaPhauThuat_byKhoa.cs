...
namespace NamDinh_QLBN.Reports.PhauThuat
{
    ...
    /// </summary>
    public partial class rptCaPhauThuat_byKhoa : DataDynamics.ActiveReports.ActiveReport3
    {
        private DateTime _tungay, _denngay;
        public string _makhoa, _Tenkhoa;
        public rptCaPhauThuat_byKhoa(DateTime tungay, DateTime  denngay,string Makhoa, string Tenkhoa)
        {
            //
            ...
            _makhoa = Makhoa;
            _Tenkhoa = Tenkhoa;
            lbKhoa.Text = _Tenkhoa;
            lblThang.Text = String.Format("Từ ngày {0:dd} Tháng {0:MM} năm {0:yyyy} đến ngày {1:dd} Tháng {1:MM} năm {1:yyyy}", _tungay, _denngay );
        }
        ...
        private void rptCaPhauThuat_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("set dateformat mdy SELECT a.SoBLVP,a.MaVaoVien,a.SoPhieuCD,a.LoaiDichVu,a.MaPT,Hoten,ThoiGianBD,TenTat as tentat1,ChanDoan_SauPT as ChanDoan_TruocPT,PhuongPhapPT_Text,Left(LoaiPhauThuat,1) As LoaiPT,b.SoHSBA,"
                ...
                + " WHERE DATEDIFF(dd,ThoiGianBD,'{0:MM/dd/yyyy}') <= 0 and DATEDIFF(dd,ThoiGianBD,'{1:MM/dd/yyyy}') >= 0 and d.makhoa = '{2}' and f.LoaiPhauThuat in (11,21,3,4) ORDER BY Left(LoaiPhauThuat,1) , ThoiGianBD ", _tungay,_denngay, _makhoa);
            this.DataSource = _ds;
