...
namespace NamDinh_QLBN.Reports.PhauThuat
{
    ...
    public partial class repSoLenThuoc_Sua : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        public repSoLenThuoc_Sua(String MaKhoa, int NhomThuoc, object Ngay, String TenKhoa, String MaPhieuDuyet)
        {
            //
        ...
        private void repSoLenThuoc_DataInitialize(object sender, EventArgs e)
        {
            ...
            SQLCmd.CommandText = String.Format("set dateformat dmy SELECT  a.Thuocid as MATHUOC,a.TENTHUOC FROM "
                    ...
                    + " inner join  "
                    + " (SELECT * FROM BENHNHAN_PT_CHIPHI WHERE DATEDIFF(DD,NGAYCHIDINH,'{0:dd/MM/yyyy}') = 0 and LOAICHIPHI IN ('D01')) CC ON CC.MaDichVu = A.ThuocID "
                    + " WHERE ", _Ngay);
            ...
            //     + " WHERE ", _Ngay);
            //SQLCmd.CommandText += String.Format(" SACLAPPHIEU.MaKhoa = '{0}' and SoThuoc.MaKhoa ='{0}'  ", _MaKhoa);
            //if (_NhomThuoc == -1)//In chung mot so
            //{
            //}
            //else
            ...
            _ds.SQL = String.Format("set dateformat dmy SELECT ISNULL(C.MAPHIEUDUYET,'') AS MAPHIEUDUYET,N'Chi phí bất thường' AS NHOM1,1 AS NHOM,B.MAVAOVIEN,A.HOTEN,YEAR(C.NGAYCHIDINH) - A.NAMSINH AS TUOI,DMKHOAPHONG.TENTAT AS TENBUONG FROM"
                   ...
                   + " WHERE DATEDIFF(DD,C.NGAYCHIDINH,'{0:dd/MM/yyyy}') = 0 AND C.MAPHIEUDUYET ='{1}' AND C.LOAICHIPHI IN ('D01')"
                   + " GROUP BY C.MAPHIEUDUYET,B.MAVAOVIEN,A.HOTEN,C.NGAYCHIDINH,A.NAMSINH,DMKHOAPHONG.TENTAT"
                   + " ORDER BY DMKHOAPHONG.TENTAT ", _Ngay, _LuotIn);
        ...
        private void repSoLenThuoc_FetchData(object sender, FetchEventArgs eArgs)
        {
            try
            {
                ...
                if (_NhomThuoc == -1)
                {
                    SQLCmd.CommandText += String.Format(" AND SACLAPPHIEU.NHOMSO <> -1 AND SACLAPPHIEU.MAKHOA='{0}'", _MaKhoa);
                }
                else
                {
                    SQLCmd.CommandText += String.Format(" AND SACLAPPHIEU.NHOMSO = {1} AND  SACLAPPHIEU.NHOMSO <> -1 AND SACLAPPHIEU.MAKHOA='{0}'", _MaKhoa, _NhomThuoc);
                }
