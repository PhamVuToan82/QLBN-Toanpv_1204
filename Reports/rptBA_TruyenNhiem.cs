...
namespace NamDinh_QLBN.Reports
{
    ...
    /// </summary>
    public partial class rptBA_TruyenNhiem : DataDynamics.ActiveReports.ActiveReport3
    {
        string _MaVaoVien, _TenKhoa, _NgayChuyen, _NgayDT,_Hsba,_LuuTru;
        int _LoaiBenhAn;
        public rptBA_TruyenNhiem(string MaVaoVien, string TenKhoa, string NgayChuyen, string NgayDT, string hsba, string Luutru, int LoaiBenhAn)
        {
            //
        ...
        {
            Subrpt_BenhAn_TongKet_NoiKhoa rpttongket = new Subrpt_BenhAn_TongKet_NoiKhoa(_MaVaoVien);
            subReport5.Report = rpttongket;
        }

        private void groupFooter2_Format(object sender, EventArgs e)
        {
            Subrpt_BenhAn_BA_TruyenNhiem rtpBenhAn = new Subrpt_BenhAn_BA_TruyenNhiem(_MaVaoVien,_LoaiBenhAn);
        ...
        private void Subrpt_ylenh_dieutri_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = string.Format(" select UPPER(c.HoTen) AS TenBenhNhan,substring(convert(nvarchar(4), (c.Namsinh)), 1, 1) as NS1,substring(convert(nvarchar(4), (c.Namsinh)), 2, 1) as NS2,substring(convert(nvarchar(4), (c.Namsinh)), 3, 1) as NS3,substring(convert(nvarchar(4), (c.Namsinh)), 4, 1) as NS4,"
                                        ...
                                        + " SUBSTRING(c.MaDanToc, 1, 1) as DanToc1, SUBSTRING(c.MaDanToc, 2, 1) as DanToc2, TenDT, SUBSTRING(DmNuocNgoai.MaNuocNgoai, 1, 1) as MaNuocNgoai1,SUBSTRING(DmNuocNgoai.MaNuocNgoai, 2, 1) as MaNuocNgoai2 , DmNuocNgoai.TenNuocNgoai As NgoaiKieu, a.Diachi,substring(convert(nvarchar(3), (b.Ma_Huyen)), 1, 1) as Ma_Huyen1,substring(convert(nvarchar(3), (b.Ma_Huyen)), 2, 1) as Ma_Huyen2,DmQuanHuyen.TenQuanHuyen,"
