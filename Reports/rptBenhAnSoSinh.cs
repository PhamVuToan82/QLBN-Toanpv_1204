...
{
    /// <summary>
    /// Summary description for rptBenhAnSoSinh.
    /// </summary>    
    public partial class rptBenhAnSoSinh : DataDynamics.ActiveReports.ActiveReport3
    {
        private string MaBenhNhan = "";
        private string LanVaoVien = "0";
        public rptBenhAnSoSinh(string _MaBenhNhan, string _LanVaoVien, string TenKhoa)
        {
            //
        ...
        private void rptBenhAnNgoai_ReportStart(object sender, EventArgs e)
        {
            ...
            SQLCmd.CommandText = string.Format("SELECT BENHNHAN.HoTen," + GlobalModuls.Global.NgayLV.Year + "-NamSinh As Tuoi,GioiTinh,MaDanToc,DMDANTOC.TenDT,TenDonVi, "
                                + " TenQH,NoiO,TenTinh,SoTheBHYT,DMDOITUONGBN.TenDT As TenDoiTuong,LienHe,NgayVaoVien,BENHNHAN_CHITIET.SoHSBA, "
                                + " BacSyKham,NgayKham,Trieuchung_VV "
            ...
            while (dr.Read())
            {
                ...
                lblGioi.Text = "3. Giới: " + ((dr["GioiTinh"].ToString() == "1") ? (" Nam") : (" Nữ"));
                lblDanToc.Text = "4. Dân tộc: " + dr["tenDT"].ToString();
                if (dr["TrieuChung_VV"].ToString().Trim() != "")
                {
                    label53.Text = "23. Phòng khám / Cấp cứu:" + dr["TrieuChung_VV"].ToString();//23. KKB, Cấp cứu: 
                }
                lblDiaChi.Text = "8. Chỗ ở: " + dr["NoiO"].ToString() + " Tỉnh, thành phố " + dr["TenTinh"];
                lblLienHe.Text = "10. Họ tên, địa chỉ người nhà khi cần báo tin: " + dr["LienHe"].ToString();
                lblNgayGiovaoVien.Text = string.Format("13. Vào viện lúc {0:HH} giờ {0:mm}   ngày {0:dd/MM/yyyy}", dr["NgayVaoVien"]);
