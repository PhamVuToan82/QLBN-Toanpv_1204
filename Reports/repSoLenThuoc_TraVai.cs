...
namespace NamDinh_QLBN.Reports
{
    ...
    /// </summary>
    public partial class repSoLenThuoc_TraVai : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        public repSoLenThuoc_TraVai(DateTime TuNgay, DateTime DenNgay, string TenKhoa)
        {
            //
        ...
        private void repSoLenThuoc_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("set dateformat dmy select X.*,isnull(SUM(b.kyquy),0) as KyQuy from( select distinct a.MaKhamBenh as MaVaoVien,TenBenhNhan as HoTen, NamSinh as Tuoi,TenKhoa,ThoigianDangky from NAMDINH_QLBN.dbo.tblbenhnhan_muonvai  a inner join NAMDINH_QLBN.dbo.tblPhieuMuon_Vai c on c.MakhamBenh = a.MaKhambenh  and c.HoanTien_M = 1 ");
            if (_TenKhoa != "Tất Cả")
            {
            ...
            _ds.SQL = _ds.SQL + string.Format(" ) X inner join  NAMDINH_QLBN.dbo.tblbenhnhan_Vai_KyQuy b on X.MaVaoVien = b.MaKhamBenh inner join (select distinct MaPhieuMuon from NAMDINH_QLBN.dbo.tblPhieuMuon_Vai where HoanTien_M =1) xx on xx.MaPhieuMuon = b.MaPhieuMuon group by x.MaVaoVien ,X.HoTen , X.Tuoi,X.TenKhoa,x.ThoigianDangky order by TenKhoa,ThoigianDangky");
            this.DataSource = _ds;
            foreach (DataDynamics.ActiveReports.TextBox _lb in detail.Controls)
            {
        ...
        private void repSoLenThuoc_FetchData(object sender, FetchEventArgs eArgs)
        {
            try
            {
                ...
                if (MaVaoVien == "") return;
                SQLCmd.CommandText = string.Format(" select MaDoVai as MADICHVU ,TenDoVai as TENTHUOC,SUM(Soluong) as SOLUONG, '' AS GhiChu from NAMDINH_QLBN.dbo.tblPhieuMuon_Vai a INNER join NAMDINH_QLBN.dbo.tblbenhnhan_muonvai b on a.MaKhamBenh = b.MaKhambenh and a.HoanTien_M =1 where a.makhambenh = '{0}'"
