...
namespace NamDinh_QLBN.Reports
{
    ...
    /// </summary>
    public partial class Subrpt_ylenh_dieutri : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        public Subrpt_ylenh_dieutri(string MaVaoVien, string MaKhoa,DateTime TuNgay, DateTime DenNgay,bool Chon)
        {
            //
        ...
        private void Subrpt_ylenh_dieutri_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = string.Format("set dateformat dmy HH:MM:SS: select a.MaVaoVien, b.SoPhieu, b.NgayChiDinh,  e.Stt ,Case when d.LoaiDichvu in ('D01','D03') then ttt.TenThuocXml +  ' x  ' + CONVERT(nvarchar(50),convert(int,sum(C.SoLuong))) + ' ' + d.DVT + '  ( ' + c.Ghichu + ' )' end as Ylenh,  "
                                    +" cs.TenCDChamSoc, b.BacSyDT, d.TenTat, KP.TenKhoa, BN.HoTen, BN.NAMSINH, A.DIACHI,case when bn.GioiTinh = 1 then N'Nam' else N'Nữ' end GioiTinnh, a.ChanDoan_KhoaDT, ttt.ThuTu_KeDon, ttt.HoatChat, ttt.TenThuocXml, Case when b.Nhom = 1 or b.CapCuu = 0 then 'BT' end as BatThuong, b.DienBienBenh as DienBien, loaidv.TenLoaiDichvu, d.LoaiDichvu, b.Ylenh as Ylenh_boxung "
                                    + " from NAMDINH_QLBN.dbo.BENHNHAN_CHITIET a inner "
                                    + " join Namdinh_qlbn.dbo.BENHNHAN_PHIEUDIEUTRI b on a.MaVaoVien = b.MaVaoVien "
                                    + " inner"
                                    + " join NAMDINH_QLBN.dbo.PHIEUDIEUTRI_CHITIET c on c.SoPhieu = b.SoPhieu"
                                    + " inner"
                                    + " join NAMDINH_QLBN.DBO.INPHIEUTHANHTOAN d on d.MaDichvu = c.MaDichVu   and(d.LoaiDichvu in ('D01', 'D03') and d.MaDichvu not in ('MAU-MA-0065', 'MAU-MA-0075', 'MAU-MA-0076', 'MAU-MA-0099'))"
