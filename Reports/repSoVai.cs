...
namespace NamDinh_QLBN.Reports
{
    ...
    /// </summary>
    public partial class repSoVai : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        public repSoVai(DateTime TuNgay, DateTime DenNgay, string TenKhoa)
        {
            //
        ...
        private void repSoLenThuoc_FetchData(object sender, FetchEventArgs eArgs)
        {
            try
            {
                ...
                //TenKhoa = Fields["TenKhoa"].Value.ToString();
                if (MaVaoVien == "") return;
                SQLCmd.CommandText = string.Format(" select MaDoVai,TenDoVai,SUM(Soluong) as SOLUONG from NAMDINH_QLBN.dbo.tblPhieuMuon_Vai a INNER join NAMDINH_QLBN.dbo.tblbenhnhan_muonvai b on a.MaKhamBenh = b.MaKhambenh where a.makhambenh = '{0}'"
                ...
                while (dr.Read())
                {
                    ...
                    foreach (DataDynamics.ActiveReports.TextBox _txt in reportFooter1.Controls)
                    {
                            ...
                            if (_txt.Name == dr["MaDoVai"].ToString())
                            {
                                if (_txt.Value == null)
                                {
