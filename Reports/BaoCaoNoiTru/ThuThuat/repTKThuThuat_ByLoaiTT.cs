...
namespace NamDinh_QLBN.Reports.BaoCaoNoiTru.ThuThuat
{
    ...
    /// </summary>
    public partial class  repTKThuThuat_ByLoaiTT : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        public repTKThuThuat_ByLoaiTT(System.DateTime TNgay, System.DateTime DNgay, String MaKhoa, String TenKhoa, bool isTatCa,
        ...
        private void repCLS_ByBS_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("SELECT B.SOHSBA,A.HOTEN,YEAR(C.NGAYCHIDINH) - A.NAMSINH AS TUOI,CASE WHEN A.GIOITINH = 0 THEN N'Nữ' ELSE N'Nam' END AS GIOITINH,"
                ...
                + " INNER JOIN DMPHAUTHUAT ON DMPHAUTHUAT.MADICHVU = D.MADICHVU AND DMPHAUTHUAT.LOAIDICHVU = D.LOAIDICHVU AND (DMPHAUTHUAT.LAPHAUTHUAT = 2 OR DMPHAUTHUAT.LAPHAUTHUAT = 3)"
            ...
            {
                _ds.SQL += string.Format(" AND DMPHAUTHUAT.LOAIPHAUTHUAT LIKE '{0}'", _MaLoaiTT);
            }
            if (_MaBS != "0")
            {
                _ds.SQL += string.Format(" AND C.MABS = {0}", _MaBS);
            }
            _ds.SQL += " ORDER BY DMLOAIPHAUTHUAT.TenLoaiPT,DMBACSY.TENBS,DMBACSY.MaBS,A.HOTEN,DMDICHVU.TENDICHVU";
            this.DataSource = _ds;
        }
