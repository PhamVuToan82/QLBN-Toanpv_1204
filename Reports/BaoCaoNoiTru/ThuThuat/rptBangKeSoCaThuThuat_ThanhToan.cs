...
namespace NamDinh_QLBN.Reports.BaoCaoNoiTru.ThuThuat
{
    ...
    /// </summary>
    public partial class  rptBangKeSoCaThuThuat_ThanhToan : DataDynamics.ActiveReports.ActiveReport3
    {
        private System.DateTime _TNgay, _DNgay;
        private String _MaKhoa, _TenKhoa;
        public rptBangKeSoCaThuThuat_ThanhToan(System.DateTime TNgay, System.DateTime DNgay, String MaKhoa, String TenKhoa)
        {
            //
        ...
        private void repCLS_ByBS_DataInitialize(object sender, EventArgs e)
        {
            ...
            DataDynamics.ActiveReports.DataSources.SqlDBDataSource _ds = new DataDynamics.ActiveReports.DataSources.SqlDBDataSource();
            _ds.ConnectionString = GlobalModuls.Global.glbConnectionString;
            _ds.SQL = String.Format("set dateformat dmy select * from (SELECT  LoaiPhauThuat as MaLoaiPT, LoaiPT, Hoten,Namsinh,dt.TenDT, e.Tentat as KhoaCD, a.MaPhieuCD, d.Chandoan, dv.Tendichvu, right(LoaiPT,2) as TenTat_LoaiPT, Ngaymo, Nguoithuchien, Gayme, Nguoiphu, TPKhac, "
            + " case P.Dathanhtoan when 1 then '' else 'X' end as Dathanhtoan  "
            + " FROM ((((THONGTIN_THUTHUAT a INNER JOIN BENHNHAN_CHITIET b ON a.MaVaoVien=b.MaVaoVien) "
            + " INNER JOIN BENHNHAN c ON b.MaBenhNhan=c.MaBenhNhan) "
            + " INNER JOIN BENHNHAN_PHIEUDIEUTRI d ON a.MaPhieuCD=d.SoPhieu) "
            ...
            + " INNER JOIN DMDOITUONGBN dt ON g.Doituong = dt.MaDT "
            + " INNER JOIN NAMDINH_SYSDB.dbo.DMDICHVU dv ON a.Madichvu = dv.Madichvu  "
            + " WHERE NgayMo >= '01/05/2022 00:00:01' and a.KhoaTH = '{0}' and b.thoigianthanhtoan  >= '{1:dd/MM/yyyy 00:00:01}' and b.thoigianthanhtoan <= '{2:dd/MM/yyyy 23:59:59}' "
            + " Union all "
            ...
            + " WHERE NgayMo >= '01/05/2022 00:00:01' and a.KhoaTH = '{0}' and b.thoigianthanhtoan  >= '{1:dd/MM/yyyy 00:00:01}' and b.thoigianthanhtoan <= '{2:dd/MM/yyyy 23:59:59}'"
            + " Union all "
            + " SELECT LoaiPhauThuat as MaLoaiPT,LoaiTT as LoaiPT, Tenbenhnhan as Hoten,Namsinh,dt.TenDT, e.Tentat as KhoaCD, a.MaPhieuCD,kqdvkham.Chandoan , dv.Tendichvu, right(LoaiTT,2) as TenTat_LoaiPT, Ngaymo, Nguoithuchien, Gayme, Nguoiphu, TPKhac,case ct.DaTinhPhi when 1 then '' else 'X' end as Dathanhtoan  "
            + " FROM ((((Namdinh_Khambenh.dbo.THONGTIN_THUTHUAT a "
            + " INNER JOIN Namdinh_Khambenh.dbo.tblKhambenh b ON a.Makhambenh=b.Makhambenh inner JOIN NAMDINH_QLBN.DBO.BENHNHAN_CHITIET CT ON CT.MaKhamBenh = B.MaKhambenh) "
            + " INNER JOIN Namdinh_Khambenh.dbo.tblBenhnhan c ON b.MaBenhNhan=c.MaBenhNhan) "
            + " INNER JOIN Namdinh_Khambenh.dbo.tblKhambenh_Chidinh d ON a.MaPhieuCD=d.MaphieuCD) "
            + " INNER JOIN Namdinh_Khambenh.dbo.tblKhambenh_dichvu P ON P.MaphieuCD = D.MaphieuCD AND P.MADICHVU = A.Madichvu AND P.DATHUCHIEN =1 "
            + " INNER JOIN DMKHOAPHONG e ON d.KhoaCD=e.MaKhoa) "
            ...
            + " INNER JOIN NAMDINH_SYSDB.dbo.DMDICHVU dv ON a.Madichvu = dv.Madichvu  "
            + " inner join NAMDINH_QLBN.dbo.tblkhambenh_kqdvkham_Max kqdvkham on kqdvkham.MaKhambenh =  b.MaKhambenh "
            + " WHERE NgayMo >= '01/05/2022 00:00:01' and a.KhoaTH = '{0}' and ct.thoigianthanhtoan  >= '{1:dd/MM/yyyy 00:00:01}' and ct.thoigianthanhtoan <= '{2:dd/MM/yyyy 23:59:59}') X"
            + " ORDER BY LoaiPT, NgayMo ", _MaKhoa, _TNgay, _DNgay);
            this.DataSource = _ds;
        }
