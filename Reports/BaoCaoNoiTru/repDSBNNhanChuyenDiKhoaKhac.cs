...
namespace NamDinh_QLBN.Reports.BaoCaoNoiTru
{
    ...
    /// </summary>
    public partial class repDSBNNhanChuyenDiKhoaKhac : DataDynamics.ActiveReports.ActiveReport3
    {
        private System.DateTime _TNgay, _DNgay;
        private String _MaKhoa;
        public repDSBNNhanChuyenDiKhoaKhac(System.DateTime TNgay, System.DateTime DNgay, String MaKhoa, String TenKhoa)
        {
            //
        ...
        private void repDSBNNhapVien_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("SELECT B.SOHSBA,A.HOTEN,YEAR(C.NGAYCHUYEN) - A.NAMSINH AS TUOI,CASE WHEN A.GIOITINH = 0 THEN N'Nữ' ELSE N'Nam' END AS GIOITINH,"
                ...
                + " INNER JOIN DMKHOAPHONG ON DMKHOAPHONG.MAKHOA = C.MAKHOA"
                + " WHERE C.MAKHOACD = '{0}'  AND DATEDIFF(dd,C.NGAYCHUYEN,'{1:MM/dd/yyyy}') <= 0 AND DATEDIFF(dd,C.NGAYCHUYEN,'{2:MM/dd/yyyy}') >= 0 AND DMKHOAPHONG.IS_KHOADIEUTRI = 1"
