...
namespace NamDinh_QLBN.Reports.BaoCaoNoiTru.NoiDKKCB_BanDau
{
    ...
    /// </summary>
    public partial class  repDS_BN_DKKCBBD_ByNoiDK : DataDynamics.ActiveReports.ActiveReport3
    {
        private System.DateTime _TNgay, _DNgay;
        private String _MaKhoa,_MaTu,_MaDen;
        private bool _NgayVaoVien, _NgayRaVien, _DangDieuTri,_TatCa;

        public repDS_BN_DKKCBBD_ByNoiDK(System.DateTime TNgay, System.DateTime DNgay, String MaKhoa, String TenKhoa,string MaTu,string MaDen,
            bool NgayVaoVien, bool NgayRaVien, bool DangDieuTri,bool TatCa)
        {
            //
        ...
        private void repCLS_ByBS_DataInitialize(object sender, EventArgs e)
        {
            ...
            _ds.SQL = String.Format("SELECT B.SOHSBA,A.HOTEN,YEAR(F.NGAYCHUYEN) - A.NAMSINH AS TUOI,CASE WHEN A.GIOITINH = 0 THEN N'Nữ' ELSE N'Nam' END AS GIOITINH,"
                ...
                + " WHERE D.DOITUONG = 1 AND C.MAKHOA = '{0}'",_MaKhoa);
            if (_NgayRaVien)
                _ds.SQL += string.Format(" AND DATEDIFF(DD,B.NGAYRAVIEN,'{0:MM/dd/yyyy}') <= 0 AND DATEDIFF(DD,B.NGAYRAVIEN,'{1:MM/dd/yyyy}') >= 0 AND C.VDARAVIEN = 1", _TNgay, _DNgay);
            if (_DangDieuTri)
            ...
            if(!_DangDieuTri)
            {
                _ds.SQL += String.Format(" UNION ALL"
                    + " SELECT B.SOHSBA,A.HOTEN,YEAR(F.NGAYCHUYEN) - A.NAMSINH AS TUOI,CASE WHEN A.GIOITINH = 0 THEN N'Nữ' ELSE N'Nam' END AS GIOITINH,"
                    + " DMDOITUONGBN.TENDT,ISNULL(D.MADOITUONGBHXH,'')+ISNULL(D.MANOICAP,'') + ISNULL(D.SOTHE,'') AS SOTHE,"
                    + " V.NGAYCHUYEN,C.CHANDOAN,N'Ðã ra viện' AS TRANGTHAI,B.DIACHI,"
                    + " E.MANOICAP,E.TENNOICAP FROM"
                    + " (BENHNHAN A INNER JOIN BENHNHAN_CHITIET B ON A.MABENHNHAN = B.MABENHNHAN)"
                    + " INNER JOIN BENHNHAN_KHOA C ON C.MAVAOVIEN = B.MAVAOVIEN "
                    + " INNER JOIN BENHNHAN_KHOA V ON V.MAVAOVIEN = B.MAVAOVIEN AND V.MAKHOACD = C.MAKHOA"
                    + " INNER JOIN VIEWDOITUONGHIENTAI D ON D.MAVAOVIEN = B.MAVAOVIEN"
                    + " INNER JOIN NAMDINH_SYSDB.DBO.DMNOIDKKCBBD_BHYT E ON E.MANOICAP = D.MANOICAP "
                    + " INNER JOIN BENHNHAN_KHOA F ON F.MAVAOVIEN = B.MAVAOVIEN AND F.LANCHUYENKHOA = 0"
                    + " INNER JOIN DMDOITUONGBN ON DMDOITUONGBN.MADT = D.DOITUONG"
            ...
            _ds.SQL += " ORDER BY E.MANOICAP,E.TENNOICAP,A.HOTEN";
            this.DataSource = _ds;
        }
