    ...
    public partial class repSoLenThuoc_Sua : DataDynamics.ActiveReports.ActiveReport3
    {
        ...
        private void repSoLenThuoc_DataInitialize(object sender, EventArgs e)
        {
            ...
            if (_NhomThuoc == -1)
            {
                SQLCmd.CommandText = String.Format("set dateformat mdy SELECT * FROM SOTHUOC  INNER JOIN [" + Global.glbDuoc + "].DBO.DANHMUCTHUOC A ON A.THUOCID = SOTHUOC.MATHUOC  "
                    ...
                    + " inner join phieudieutri_chitiet ct on  ct.SoPhieu = aa.SoPhieu AND CT.MADICHVU NOT IN ('ONG-TCQ-0005','ONG-TCQ-0006','ONG-TCQ-0003')  "
            ...
            _ds.SQL = String.Format("set dateformat mdy "
                    ...
                    + " INNER JOIN PHIEUDIEUTRI_CHITIET CT ON CT.SOPHIEU = DD.SOPHIEU AND CT.MADICHVU NOT IN ('ONG-TCQ-0005','ONG-TCQ-0006','ONG-TCQ-0003') AND CT.LOAIDICHVU in ('D01','D06') "
        ...
        private void repSoLenThuoc_FetchData(object sender, FetchEventArgs eArgs)
        {
            try
            {
                ...
                if (Nhom == "") return;
                SQLCmd.CommandText = "set dateformat mdy SELECT BB.MADICHVU,BB.TENTHUOC,SUM(BB.SOLUONG) AS SOLUONG,BB.GHICHU FROM (";
                if (_NhomThuoc == -1)
                {
                    SQLCmd.CommandText += String.Format(" SELECT EE.MADICHVU,EE.SOLUONG,THUOC.TENTHUOC,EE.GHICHU FROM "
                           ...
                           + " INNER JOIN PHIEUDIEUTRI_CHITIET CT ON CT.SOPHIEU = DD.SOPHIEU AND CT.MADICHVU NOT IN ('ONG-TCQ-0005','ONG-TCQ-0006','ONG-TCQ-0003') "
                           + " where ((ct.LoaiDichVu = 'D01' or ct.LoaiDichVu = 'D06') and CT.MaPhieuDuyet= '" + _LuotIn + "')  "
                           //+ " WHERE (CT.LOAIDICHVU = 'D01' and ct.MaPhieuDuyet = '" + _LuotIn + "') "
                ...
                SQLCmd.CommandText += ") BB GROUP BY MADICHVU,TENTHUOC,GHICHU";
                dr = SQLCmd.ExecuteReader();
                bool DaCo = false;
                lblGhiChu.Text = "";
                foreach (DataDynamics.ActiveReports.TextBox _lb in detail.Controls)
