...
namespace FilterRow
{
	...
	public class FilterRow
	{
		...
		private void _flex_KeyDown(object sender, KeyEventArgs e)
		{
			...
			if (e.KeyCode == Keys.F2 || (e.KeyCode == Keys.Up && _flex.Row == _row+1))
			{
		...
		private void UpdateFilter()
		{
			...
			try
			{
				...
				C1.Win.C1FlexGrid.AutoSizeFlags fl=new C1.Win.C1FlexGrid.AutoSizeFlags();
				_flex.AutoSizeRows(1,0,_flex.Rows.Count-1,_flex.Cols.Count-1,0,fl);
				_flex.AutoSizeCols(1,0,_flex.Rows.Count-1,_flex.Cols.Count-1,0,fl);
			}
			catch
			{
				_flex[_row, 0] = "Err";
				MessageBox.Show("Không xây dựng được điều kiện lọc","Thông báo",MessageBoxButtons.OK,MessageBoxIcon.Error);
			}
		...
		private string BuildFilterExpression(int col, string expr)
		{
			...
			{
				switch (_flex.Cols[col].DataType.Name )
				{
					case "String":
					...
					case "DateTime":
						if (expr.Length<10)
						{
							MessageBox.Show("Nhập sai định dạng ngày tháng ! \r\n [Ngày (2 ký tự)] / [Tháng (2 ký tự)] / [Năm (4 ký tự)]","Thông báo",MessageBoxButtons.OK,MessageBoxIcon.Error);
							return "";
						}
						expr = GlobalModuls.Global.ConvertDate(expr);
				...
				//                   ? string.Format(" like '{0}*'", ConvertDate)
			...
			switch (_flex.Cols[col].DataType.Name )
			{
				case "DateTime":
					...
					tmp=expr.Substring(expr.Length-10);
					tmp= GlobalModuls.Global.ConvertDate(tmp);
					tmp=expr.Substring(0,expr.Length-10)+tmp;
					break;
				default:
					tmp= expr;
					break;
			...
			{
				if (oper.IndexOf(tmp[index]) < 0)
				{
					string retval = tmp.Substring(0, index) + " ";
					retval += string.Format("'{0}'", tmp.Substring(index).Trim());
					return retval;
				}
			}
